...
function initProgressBar() {
	...
	function progressBarStart(el = []) {

		const progressBars = el.length == 0 ? document.querySelectorAll('[data-cpb]') : el;
		if (progressBars.length > 0) {
			...
			progressBars.forEach(progressBar => {
				...
				if (setting) {
					...
					const animateStop = progressBar.getAttribute('data-cpb-animate-stop') != null ? true : false;
	...
	let startElements = document.querySelectorAll('[data-cpb-animate-stop]');
		...
		startElements.forEach(el => {
			el.addEventListener('click', function (e) {
				if (el.getAttribute('data-cpb-animate-stop') != null) {
					...
					el.setAttribute('data-cpb-animate', '3000,0,easeInOutBack');

					progressBarStart([el]);
					el.removeAttribute('data-cpb-animate');
				}
			});
		});
	}
